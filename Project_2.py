...
def find_urls(s):
    url_lst = []
    x = re.findall('(ht\S+?.[^ ]*)\s',s)
    for link in x:
        if '.' not in link:
            x.remove(link)
    return(x)

## PART 2  - Defin\e a function grab_headlines.
## INPUT: N/A. No input.
...
def grab_headlines():
    ...
    tags = soup.find_all('div',{'class':'view view-most-read view-id-most_read view-display-id-panel_pane_1 view-dom-id-99658157999dd0ac5aa62c2b284dd266'})
    for tag in tags:
        headline = ((tag.text).strip())
        headline_lst.append(headline)

    for headlines in headline_lst:
...
def get_umsi_data():
    ...
    pgs_soup = soup.find_all('li',{'class':'pager-current'})
    pg_num = 0
    for pg in pgs_soup:
        pg_num = int((pg.text).split('of')[1])

    #loop through pages
    for page in range(pg_num):
        url = 'https://www.si.umich.edu/directory?field_person_firstname_value=&amp%3Bfield_person_lastname_value=&amp%3Brid=5&page=' + str(page)
        r = requests.get(url,headers = {'User-Agent':'SI_CLASS'})
        soup = BeautifulSoup(r.text,'html.parser')
        
        #get names
        name_tags = soup.find_all('div', {'class':"field field-name-title field-type-ds field-label-hidden"})
        for tag in name_tags:
            name = tag.text.strip()
            name_lst.append(name)

        #get positions
        position_tags = soup.find_all('div',{'class':"field field-name-field-person-titles field-type-text field-label-hidden"})
        for tag in position_tags:
            position = tag.text.strip()
            position_lst.append(position)
    
    umsi_titles= dict(zip(name_lst,position_lst))
    return(umsi_titles)
    ### umsi_titles is the exact same as data in the test case, except for an extra space in the position of people in the directory who don't have a title. Spoke to Chong about this in office hours, he said it is ok and passes the test. 
